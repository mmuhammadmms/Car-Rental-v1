    ...
    <div class="col-sm-9">
      ...
      <div class="jumbotron">
        <?php
          ...
          $sql = "select CUST_USERNAME from customer where CUST_USERNAME = '$uname'";
          $stid = oci_parse($conn, $sql);
          oci_execute($stid);
          $row2 = oci_fetch_row($stid);

          $sql = "select STAFF_USERNAME from STAFF where STAFF_USERNAME = '$uname'";
          $stid = oci_parse($conn, $sql);
          oci_execute($stid);
          $row3 = oci_fetch_row($stid);

          $sql = "select CUST_EMAIL from CUSTOMER where CUST_EMAIL = '$eml'";
          $stid = oci_parse($conn, $sql);
          oci_execute($stid);
          $row4 = oci_fetch_row($stid);

          $sql = "select STAFF_EMAIL from STAFF where STAFF_EMAIL = '$eml'";
          $stid = oci_parse($conn, $sql);
          oci_execute($stid);
          $row5 = oci_fetch_row($stid);



          if ($row2 > 0 || $row3 > 0){
            echo ("<SCRIPT LANGUAGE='JavaScript'>
           window.alert('Username is already taken.Please enter again.')
           window.location.href='addusers.php';
           </SCRIPT>");

         }else if ($row4 > 0 || $row5 >0){
           echo ("<SCRIPT LANGUAGE='JavaScript'>
          window.alert('Email is already taken.Please enter again.')
          window.location.href='addusers.php';
          </SCRIPT>");
         }else{
          ...
          if ($role == 'staff'){
            $sql2 = "select 'S'||to_char(cust_seq.nextval,'FM000') from dual";
            $sql = "INSERT INTO staff VALUES ( :ai , :username , :fname, :lname, :password, :ic , :email , :cnumber , :country ,
            :city , :state , :poscode  , :address , sysdate)";
          }else if ($role == 'customer'){
            $sql2 = "select 'C'||to_char(staff_seq.nextval,'FM000') from dual";
            $sql = "INSERT INTO customer VALUES ( :ai , :username , :fname, :lname, :password, :ic , :email , :cnumber , :country ,
            :city , :state , :poscode , :address , sysdate)";
          }
          ...
          oci_bind_by_name($stmt, ':username', $_POST["usname"]);
          ...
          oci_bind_by_name($stmt, ':poscode', $_POST["poscode"]);
