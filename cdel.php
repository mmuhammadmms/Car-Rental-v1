    ...
    <h4>ADD CARS</h4>
      ...
      <div class="jumbotron"><center>
            ...
            <?php
                      ...
                      $sql = "UPDATE cars SET cars_status = 'Unavailable' where cars_id = :cid";
                      $stmt = oci_parse($conn, $sql);
                      oci_bind_by_name($stmt, ':cid', $cid);
                      $result = oci_execute($stmt);

                      $sql = "DELETE FROM carsimage where cars_id = :cid";
                      $stmt = oci_parse($conn, $sql);
                      oci_bind_by_name($stmt, ':cid', $cid);
                      $result = oci_execute($stmt);
                      ...
                      if (!$result){
                        echo ("<SCRIPT LANGUAGE='JavaScript'>
    window.alert('Unable to delete.Car is being rented.')
    window.location.href='adcars.php';
    </SCRIPT>");
                      }

             ?>

<p>Cars <?php echo " " . $cid . " " ;?> has been deleted.</p>
    </center>
      </div>
    </div>
