...
<div class="container-fluid">
  ...
  <li><a href="profile.php">My Profile</a></li>
    <li><a href="mreservation.php">My Reservation</a></li>
    <li class="active"><a href="mpayment.php">My Payment</a></li>
    <li><a href="mrent.php">Rent Car</a></li>
    ...
    <div class="col-sm-9">
      ...
      <h4>Notice</h4>
        <div class="jumbotron"><center>
          ...
          $sql5 = "select 'TXN'||to_char(TXN_seq.nextval,'FM0000') from dual";
          $stid5 = oci_parse($conn, $sql5);
          oci_execute($stid5);
          $row5 = oci_fetch_row($stid5);
          $txn = $row5[0];

          $item_number = $_SESSION['item_number'];
          $txn_id = $txn;
          $payment_gross = $_SESSION['amount'];
          ...
          if(!empty($txn_id) ){
                  ...
                  oci_bind_by_name($stid3, ':username', $id);
                  oci_execute($stid3);
                  $row3 = oci_fetch_row($stid3);
                  $cid = $row3[0];

                  $sql = "INSERT INTO payments VALUES ( pay_seq.nextval , :it , :id ,:txn , :pg , :cd , :ps , sysdate , 'Full'  )";
                  $stmt = oci_parse($conn, $sql);
                  oci_bind_by_name($stmt, ':it', $item_number );
                  oci_bind_by_name($stmt, ':txn', $txn_id  );
                  oci_bind_by_name($stmt, ':id', $cid );
                  oci_bind_by_name($stmt, ':pg', $payment_gross );
                  oci_bind_by_name($stmt, ':cd', $currency_code );
                  oci_bind_by_name($stmt, ':ps', $payment_status );
                  oci_execute($stmt);

                  $sql2 = "UPDATE reservation SET payment_status = 'Paid' WHERE reservation_id = :it ";
                  $stmt2 = oci_parse($conn, $sql2);
                  oci_bind_by_name($stmt2, ':it', $item_number );
                  oci_execute($stmt2);
          ...
          ?>
<p>Your full price payment is successful.</p>
Thank you .If you have any questions, please contact us.
<meta http-equiv="refresh" content="7; url=mpayment.php" />
      </center>
        </div>
     </div>
